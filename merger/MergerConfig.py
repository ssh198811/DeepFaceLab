...
class MergerConfigMasked(MergerConfig):
    ...
    def ask_settings(self):
        ...
        io.log_info("choose merge mode : " + str(UIParamReflect.UIParam2Config.merger_mode))
        InfoNotifier.InfoNotifier.g_progress_info.append(f"合并模型 {UIParamReflect.UIParam2Config.merger_mode}")

        mode = UIParamReflect.UIParam2Config.merger_mode
        ...
        if 'raw' not in self.mode:
            if self.mode == 'hist-match':
                ...
                self.masked_hist_match = UIParamReflect.UIParam2Config.masked_hist_match

            if self.mode == 'hist-match' or self.mode == 'seamless-hist-match':
                #self.hist_match_threshold = np.clip ( io.input_int("Hist match threshold", 255, add_info="0..255"), 0, 255)
                self.hist_match_threshold = UIParamReflect.UIParam2Config.hist_match_threshold
        ...
        if self.face_type == FaceType.FULL:
            ...
            self.mask_mode = UIParamReflect.UIParam2Config.mask_mode
            io.log_info("choose mask mode : " + str(UIParamReflect.UIParam2Config.mask_mode))

            InfoNotifier.InfoNotifier.g_progress_info.append(f"遮罩模型 {UIParamReflect.UIParam2Config.mask_mode}")
        else:
            s = """Choose mask mode: \n"""
        ...
        if 'raw' not in self.mode:
            ...
            self.erode_mask_modifier = UIParamReflect.UIParam2Config.erode_mask_modifier
            self.blur_mask_modifier = UIParamReflect.UIParam2Config.blur_mask_modifier
            self.motion_blur_power = UIParamReflect.UIParam2Config.motion_blur_power
            io.log_info("Choose erode mask modifier 0" + str(self.erode_mask_modifier))
            io.log_info("Choose blur mask modifier 0" + str(self.blur_mask_modifier))
            io.log_info("Choose motion blur power 0" + str(self.motion_blur_power))

            InfoNotifier.InfoNotifier.g_progress_info.append(f"erode mask modifier {self.erode_mask_modifier}")
            InfoNotifier.InfoNotifier.g_progress_info.append(f"blur mask modifier{self.blur_mask_modifier}")
            InfoNotifier.InfoNotifier.g_progress_info.append(f"motion blur power {self.motion_blur_power}")

        #self.output_face_scale = np.clip (io.input_int ("Choose output face scale modifier", 0, add_info="-50..50" ), -50, 50)
        self.output_face_scale = UIParamReflect.UIParam2Config.output_face_scale
        io.log_info("Choose output face scale modifier" + str(self.output_face_scale))
        InfoNotifier.InfoNotifier.g_progress_info.append(f"face scale modifier {self.output_face_scale}")

        if 'raw' not in self.mode:
           # self.color_transfer_mode = io.input_str ( "Color transfer to predicted face", None, valid_list=list(ctm_str_dict.keys())[1:] )
            self.color_transfer_mode = UIParamReflect.UIParam2Config.color_transfer_mode
            io.log_info("Color transfer to predicted face " + str(self.color_transfer_mode))
            self.color_transfer_mode = ctm_str_dict[self.color_transfer_mode]
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Color transfer to predicted face {self.color_transfer_mode}")
        super().ask_settings()

        self.super_resolution_power = UIParamReflect.UIParam2Config.super_resolution_power
        ...
        io.log_info("Choose super resolution power " + str(self.super_resolution_power))
        InfoNotifier.InfoNotifier.g_progress_info.append(f"super resolution power {self.super_resolution_power}")
        if 'raw' not in self.mode:
            ...
            self.image_denoise_power = UIParamReflect.UIParam2Config.image_denoise_power
            self.bicubic_degrade_power = UIParamReflect.UIParam2Config.bicubic_degrade_power
            self.color_degrade_power = UIParamReflect.UIParam2Config.color_degrade_power

            io.log_info("Choose image degrade by denoise power " + str(self.image_denoise_power))
            io.log_info("Choose image degrade by bicubic rescale power " + str(self.bicubic_degrade_power))
            io.log_info("Degrade color power of final image " + str(self.color_degrade_power))
            InfoNotifier.InfoNotifier.g_progress_info.append(f"image degrade by denoise power {self.image_denoise_power}")
            InfoNotifier.InfoNotifier.g_progress_info.append(f"super image degrade by bicubic rescale power {self.bicubic_degrade_power}")
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Degrade color power of final image {self.color_degrade_power}")
        io.log_info ("")
