...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            ...
            resolution = UIParamReflect.UIParam2Config.Resolution
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Resolution select :{resolution}")
            ...
            self.options['face_type'] = UIParamReflect.UIParam2Config.face_type
            InfoNotifier.InfoNotifier.g_progress_info.append(f"face_type select :{UIParamReflect.UIParam2Config.face_type}")
            ...
            self.options['archi'] = UIParamReflect.UIParam2Config.ae_arch
            InfoNotifier.InfoNotifier.g_progress_info.append(f"AE architecture select :{UIParamReflect.UIParam2Config.ae_arch}")
        ...
        if self.is_first_run():
            ...
            self.options['ae_dims'] = UIParamReflect.UIParam2Config.auto_encoder_dim
            InfoNotifier.InfoNotifier.g_progress_info.append(f"AutoEncoder dimensions select :{UIParamReflect.UIParam2Config.auto_encoder_dim}")

            # e_dims = np.clip ( io.input_int("Encoder dimensions", default_e_dims, add_info="16-256", help_message="More dims help to recognize more facial features and achieve sharper result, but require more VRAM. You can fine-tune model size to fit your GPU." ), 16, 256 )
            e_dims = UIParamReflect.UIParam2Config.encoder_dim
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Encoder dimensions select :{e_dims}")
            ...
            d_dims = UIParamReflect.UIParam2Config.decoder_dim
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Decoder dimensions select :{d_dims}")
            self.options['d_dims'] = d_dims + d_dims % 2

            # d_mask_dims = np.clip ( io.input_int("Decoder mask dimensions", default_d_mask_dims, add_info="16-256", help_message="Typical mask dimensions = decoder dimensions / 3. If you manually cut out obstacles from the dst mask, you can increase this parameter to achieve better quality." ), 16, 256 )
            d_mask_dims = UIParamReflect.UIParam2Config.decoder_mask_dim
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Decoder mask dimensions select :{d_mask_dims}")
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['learn_mask'] = UIParamReflect.UIParam2Config.learn_mask
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Learn mask select :{UIParamReflect.UIParam2Config.learn_mask}")

            self.options['eyes_prio'] =  UIParamReflect.UIParam2Config.eyes_priority
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Eyes priority select :{UIParamReflect.UIParam2Config.eyes_priority}")
        ...
        if self.is_first_run() or ask_override:
            if len(device_config.devices) == 1:
                self.options['models_opt_on_gpu'] = UIParamReflect.UIParam2Config.place_models_and_optimizer_on_GPU
                InfoNotifier.InfoNotifier.g_progress_info.append(f"Place models and optimizer on GPU select :{UIParamReflect.UIParam2Config.place_models_and_optimizer_on_GPU}")
            ...
            self.options['lr_dropout'] = UIParamReflect.UIParam2Config.use_learning_rate_dropout
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Use learning rate dropout select :{UIParamReflect.UIParam2Config.use_learning_rate_dropout}")
            ...
            self.options['random_warp'] = UIParamReflect.UIParam2Config.enable_random_warp_of_samples
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Enable random warp of samples select :{UIParamReflect.UIParam2Config.enable_random_warp_of_samples}")
            ...
            self.options['gan_power'] = UIParamReflect.UIParam2Config.GAN_power
            InfoNotifier.InfoNotifier.g_progress_info.append(f"GAN power select :{UIParamReflect.UIParam2Config.GAN_power}")

            if 'df' in self.options['archi']:
                self.options['true_face_power'] = UIParamReflect.UIParam2Config.true_face_power
                InfoNotifier.InfoNotifier.g_progress_info.append(f"'True face' power select :{UIParamReflect.UIParam2Config.true_face_power}")
            ...
            if self.options['face_type'] != 'wf':
                self.options['face_style_power'] = UIParamReflect.UIParam2Config.face_style_power
                InfoNotifier.InfoNotifier.g_progress_info.append(f"Face style power select :{UIParamReflect.UIParam2Config.face_style_power}")
                ...
                self.options['bg_style_power'] = UIParamReflect.UIParam2Config.background_style_power
                InfoNotifier.InfoNotifier.g_progress_info.append(f"Face style power select :{UIParamReflect.UIParam2Config.background_style_power}")
            ...
            self.options['ct_mode'] = UIParamReflect.UIParam2Config.color_transfer_for_src_faceset
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Color transfer for src faceset select :{UIParamReflect.UIParam2Config.color_transfer_for_src_faceset}")

            # self.options['clipgrad'] = io.input_bool ("Enable gradient clipping", default_clipgrad, help_message="Gradient clipping reduces chance of model collapse, sacrificing speed of training.")
            self.options['clipgrad'] = UIParamReflect.UIParam2Config.enable_gradient_clipping
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Enable gradient clipping select :{UIParamReflect.UIParam2Config.enable_gradient_clipping}")
            ...
            self.options['pretrain'] = UIParamReflect.UIParam2Config.enable_pretrain_model
            InfoNotifier.InfoNotifier.g_progress_info.append(f"Enable pretraining mode select :{UIParamReflect.UIParam2Config.enable_pretrain_model}")
    ...
    def on_initialize(self):
        ...
        for model, filename in io.progress_bar_generator(self.model_filename_list, "Initializing models"):
            ...
            if do_init:
                model.init_weights()
            InfoNotifier.InfoNotifier.g_progress_info.append("加载模型权重中...")
