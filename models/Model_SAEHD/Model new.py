...
class SAEHDModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        class Decoder(nn.ModelBase):
            def on_build(self, in_ch, d_ch, d_mask_ch, is_hd ):
                ...
                self.out_convm_struct = nn.Conv2D( d_mask_ch*2, 1, kernel_size=1, padding='SAME', kernel_initializer=conv_kernel_initializer)
            ...
            def get_weights_ex(self, include_mask):
                ...
                if include_mask:
                    weights += self.upscalem0.get_weights() + self.upscalem1.get_weights() + self.upscalem2.get_weights() \
                               + self.out_convm.get_weights() + self.out_convm_struct.get_weights()
                return weights
            ...
            def forward(self, inp):
                ...
                return tf.nn.sigmoid(self.out_conv(x)), \
                       ...
                       tf.nn.sigmoid(self.out_convm_struct(m)), \
        ...
        with tf.device ('/CPU:0'):
            ...
            self.target_srcm_struct = tf.placeholder (nn.tf_floatx, mask_shape)
            
            self.target_dstm = tf.placeholder (nn.tf_floatx, mask_shape)
            self.target_dstm_struct = tf.placeholder (nn.tf_floatx, mask_shape)
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    with tf.device(f'/CPU:0'):
                        ...
                        gpu_target_srcm_struct = self.target_srcm_struct[batch_slice,:,:,:]
                        gpu_target_dstm  = self.target_dstm  [batch_slice,:,:,:]
                        gpu_target_dstm_struct = self.target_dstm_struct[batch_slice,:,:,:]

                    # process model tensors
                    if 'df' in archi:
                        gpu_src_code     = self.inter(self.encoder(gpu_warped_src))
                        gpu_dst_code     = self.inter(self.encoder(gpu_warped_dst))
                        gpu_pred_src_src, gpu_pred_src_srcm, gpu_pred_src_srcm_struct = self.decoder_src(gpu_src_code)
                        gpu_pred_dst_dst, gpu_pred_dst_dstm, gpu_pred_dst_dstm_struct = self.decoder_dst(gpu_dst_code)
                        gpu_pred_src_dst, gpu_pred_src_dstm, gpu_pred_src_dstm_struct = self.decoder_src(gpu_dst_code)
                        ...
                        gpu_pred_src_src, gpu_pred_src_srcm, gpu_pred_src_srcm_struct = self.decoder(gpu_src_code)
                        gpu_pred_dst_dst, gpu_pred_dst_dstm, gpu_pred_dst_dstm_struct = self.decoder(gpu_dst_code)
                        gpu_pred_src_dst, gpu_pred_src_dstm, gpu_pred_src_dstm_struct = self.decoder(gpu_src_dst_code)
                    ...
                    gpu_pred_src_dstm_list.append(gpu_pred_src_dstm_struct)
                    ...
                    if learn_mask:
                        ...
                        gpu_src_loss += tf.reduce_mean ( 10*tf.square( gpu_target_srcm_struct - gpu_pred_src_srcm_struct ),axis=[1,2,3] )
                    ...
                    if learn_mask:
                        ...
                        gpu_src_loss += tf.reduce_mean ( 10*tf.square( gpu_target_dstm_struct - gpu_pred_dst_dstm_struct ),axis=[1,2,3] )
            ...
            def src_dst_train(warped_src, target_src, target_srcm, target_srcm_struct, \
                              warped_dst, target_dst, target_dstm, target_dstm_struct):
                s, d, _ = nn.tf_sess.run ( [ src_loss, dst_loss, src_dst_loss_gv_op],
                                            feed_dict={self.warped_src        :warped_src,
                                                       self.target_src        :target_src,
                                                       self.target_srcm       :target_srcm,
                                                       self.target_srcm_struct:target_srcm_struct,
                                                       self.warped_dst        :warped_dst,
                                                       self.target_dst        :target_dst,
                                                       self.target_dstm       :target_dstm,
                                                       self.target_dstm_struct:target_dstm_struct,
                                                       })
                s = np.mean(s)
                d = np.mean(d)
                return s, d
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, random_ct_samples_path=random_ct_samples_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                        ...
                        output_sample_types = [ {'types' : (t_img_warped, face_type, t.MODE_BGR),                  'data_format':nn.data_format, 'resolution': resolution, 'ct_mode': self.options['ct_mode'] },
                                                ...
                                                {'types' : (t.IMG_TRANSFORMED, face_type, t.MODE_FACE_MASK_STRUCT),'data_format':nn.data_format, 'resolution': resolution } ],
    ...
    def onTrainOneIter(self):
        ( (warped_src, target_src, target_srcm, target_srcm_struct), \
          (warped_dst, target_dst, target_dstm, target_dstm_struct) ) = self.generate_next_samples()

        src_loss, dst_loss = self.src_dst_train (warped_src, target_src, target_srcm, target_srcm_struct, warped_dst, target_dst, target_dstm, target_dstm_struct)
    ...
    def onGetPreview(self, samples):
        ( (warped_src, target_src, target_srcm, target_srcm_struct),
          (warped_dst, target_dst, target_dstm, target_dstm_struct) ) = samples
