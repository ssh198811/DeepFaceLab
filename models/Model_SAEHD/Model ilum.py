...
class SAEHDModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        class Encoder(nn.ModelBase):
            def on_build(self, in_ch, e_ch, is_hd):
                self.is_hd=is_hd
                if self.is_hd:
                    self.down1 = DownscaleBlock(in_ch, e_ch*2, n_downscales=4, kernel_size=3, dilations=1, subpixel=False)
                    self.down2 = DownscaleBlock(in_ch, e_ch*2, n_downscales=4, kernel_size=5, dilations=1, subpixel=False)
                    self.down3 = DownscaleBlock(in_ch, e_ch//2, n_downscales=4, kernel_size=5, dilations=2, subpixel=False)
                    self.down4 = DownscaleBlock(in_ch, e_ch//2, n_downscales=4, kernel_size=7, dilations=2, subpixel=False)
                else:
        ...
        self.illum_power = illum_power = 0.0001
        ...
        with tf.device (models_opt_device):
            ...
            if self.is_training:
                if gan_power != 0:
                    self.D_src = nn.PatchDiscriminator(patch_size=resolution//16, in_ch=output_ch, base_ch=512, conv_kernel_initializer=conv_kernel_initializer, name="D_src")
                    self.D_dst = nn.PatchDiscriminator(patch_size=resolution//16, in_ch=output_ch, base_ch=512, conv_kernel_initializer=conv_kernel_initializer, name="D_dst")
                    self.model_filename_list += [ [self.D_src, 'D_src.npy'] ]
                    self.model_filename_list += [ [self.D_dst, 'D_dst.npy'] ]
                    
                if illum_power != 0:
                    self.D_illum = nn.IllumDiscriminator(patch_size=resolution//16, in_ch=output_ch, base_ch=64, conv_kernel_initializer=conv_kernel_initializer, name="D_illum")
                    self.model_filename_list += [ [self.D_illum, 'D_illum.npy'] ]
                ...
                if illum_power != 0:
                    self.D_illum_opt = nn.TFRMSpropOptimizer(lr=lr, lr_dropout=lr_dropout, clipnorm=clipnorm, name='D_illum_opt')
                    self.D_illum_opt.initialize_variables ( self.D_illum.get_weights(), vars_on_cpu=optimizer_vars_on_cpu)
                    self.model_filename_list += [ (self.D_illum_opt, 'D_illum_opt.npy') ]
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    def DLoss(labels,logits):
                        mean_axes = [*range(1, logits.shape.rank)]
                        return tf.reduce_mean( tf.nn.sigmoid_cross_entropy_with_logits(labels=labels, logits=logits), axis=mean_axes)
                    ...
                    if illum_power != 0:
                        gpu_pred_src_dst_d       = self.D_illum (gpu_psd_target_dst_masked)
                        gpu_pred_src_dst_d_ones  = tf.ones_like (gpu_pred_src_dst_d)
                        gpu_pred_src_dst_d_zeros = tf.zeros_like(gpu_pred_src_dst_d)
                        gpu_target_dst_d         = self.D_illum(gpu_target_dst_masked)
                        gpu_target_dst_d_ones    = tf.ones_like(gpu_target_dst_d)

                        gpu_D_illum_loss = (DLoss(gpu_target_dst_d_ones   , gpu_target_dst_d) + \
                                            DLoss(gpu_pred_src_dst_d_zeros, gpu_pred_src_dst_d) ) * 0.5

                        gpu_D_illum_loss_gvs += [ nn.tf_gradients (gpu_D_illum_loss, self.D_illum.get_weights() ) ]

                        gpu_G_loss += illum_power*DLoss(gpu_pred_src_dst_d_ones, gpu_pred_src_dst_d)
            ...
            with tf.device (models_opt_device):
                ...
                if illum_power != 0:
                    src_D_illum_loss_gv_op = self.D_illum_opt.get_update_op (nn.tf_average_gv_list(gpu_D_illum_loss_gvs) )
            ...
            if illum_power != 0:
                def D_illum_train(warped_dst, target_dst, target_dstm):
                    nn.tf_sess.run ([src_D_illum_loss_gv_op], feed_dict={self.warped_dst :warped_dst,
                                                                         self.target_dst :target_dst,
                                                                         self.target_dstm:target_dstm})
                self.D_illum_train = D_illum_train
    ...
    def onTrainOneIter(self):
        ...
        if self.illum_power != 0:
            self.D_illum_train (warped_dst, target_dst, target_dstm)
