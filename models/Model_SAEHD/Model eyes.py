...
class SAEHDModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        with tf.device ('/CPU:0'):
            ...
            self.target_srcm_eyes = tf.placeholder (nn.tf_floatx, mask_shape)
            self.target_dstm_eyes = tf.placeholder (nn.tf_floatx, mask_shape)
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    with tf.device(f'/CPU:0'):
                        ...
                        gpu_target_srcm_eyes = self.target_srcm_eyes[batch_slice,:,:,:]                        
                        gpu_target_dstm_eyes = self.target_dstm_eyes[batch_slice,:,:,:]
            ...
            def src_dst_train(warped_src, target_src, target_srcm, target_srcm_eyes, \
                              warped_dst, target_dst, target_dstm, target_dstm_eyes):
                s, d, _ = nn.tf_sess.run ( [ src_loss, dst_loss, src_dst_loss_gv_op],
                                            feed_dict={self.warped_src :warped_src,
                                                       self.target_src :target_src,
                                                       self.target_srcm:target_srcm,
                                                       self.target_srcm_eyes:target_srcm_eyes,
                                                       self.warped_dst :warped_dst,
                                                       self.target_dst :target_dst,
                                                       self.target_dstm:target_dstm,
                                                       self.target_dstm_eyes:target_dstm_eyes,
                                                       })
                s = np.mean(s)
                d = np.mean(d)
                return s, d
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, random_ct_samples_path=random_ct_samples_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                        ...
                        output_sample_types = [ {'types' : (t_img_warped, face_type, t.MODE_BGR),      'data_format':nn.data_format, 'resolution': resolution, 'ct_mode': self.options['ct_mode'] },
                                                ...
                                                {'types' : (t.IMG_TRANSFORMED, face_type, t.MODE_FACE_MASK_EYES_HULL), 'data_format':nn.data_format, 'resolution': resolution },
                                              ],
    ...
    def onTrainOneIter(self):
        ( (warped_src, target_src, target_srcm, target_srcm_eyes), \
          (warped_dst, target_dst, target_dstm, target_dstm_eyes) ) = self.generate_next_samples()

        src_loss, dst_loss = self.src_dst_train (warped_src, target_src, target_srcm, target_srcm_eyes, warped_dst, target_dst, target_dstm, target_dstm_eyes)
    ...
    def onGetPreview(self, samples):
        ( (warped_src, target_src, target_srcm, target_srcm_eyes),
          (warped_dst, target_dst, target_dstm, target_dstm_eyes) ) = samples
