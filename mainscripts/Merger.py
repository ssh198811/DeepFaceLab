...
class MergeSubprocessor(Subprocessor):
    ...
    def __init__(self, is_interactive, merger_session_filepath, predictor_func, predictor_input_shape, merger_config, frames, frames_root_path, output_path, output_mask_path, model_iter):
        ...
        if session_data is None:
            for filename in pathex.get_image_paths(self.output_path): #remove all images in output_path
                ...
                InfoNotifier.InfoNotifier.g_progress_info.append(f"删除原文件{filename}")
    ...
    def on_result (self, host_dict, pf_sent, pf_result):
        ...
        InfoNotifier.InfoNotifier.g_progress_info.append(f"已生成 {frame.output_filepath}")
        QApplication.processEvents()
...
def main (model_class_name=None,
    ...
    try:
        ...
        io.log_info("Defalut not Use interactive merger\n")
        is_interactive = UIParamReflect.UIParam2Config.b_use_interactive_merger
        ...
        if cfg.type == MergerConfig.TYPE_MASKED:
            ...
            for filepath, dflimg in generator():
                ...
                InfoNotifier.InfoNotifier.g_progress_info.append(f"加载目标帧:{filepath}")
                QApplication.processEvents()
            ...
            if multiple_faces_detected:
                ...
                for i in io.progress_bar_generator( range(len(frames)) , "Computing motion vectors"):
                    compute_percent = round((i / frames_len) * 100.0,2)
                    InfoNotifier.InfoNotifier.g_progress_info.append(f"计算偏离度 已完成 {compute_percent} %")
                    QApplication.processEvents()
            ...
            """
        InfoNotifier.InfoNotifier.g_progress_info.append(f"计算偏离度 已完成")
        QApplication.processEvents()
        if len(frames) == 0:
