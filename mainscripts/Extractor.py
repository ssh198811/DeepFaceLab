...
class ExtractSubprocessor(Subprocessor):
    ...
    def on_result (self, host_dict, data, result):
        if self.type == 'landmarks-manual':
            ...
            io.progress_bar_inc(1)
            InfoNotifier.InfoNotifier.g_progress_info.append(str(result.filepath)+" 提取完毕!")
...
def main(detector=None,
    ...
    if output_path.exists():
        if not manual_output_debug_fix and input_path != output_path:
            ...
            if len(output_images_paths) > 0:
                ...
                InfoNotifier.InfoNotifier.g_progress_info.append("\n输出目录: "+str(output_path)+" 内不为空，将被清空!")
    ...
    if images_found != 0:
        if detector == 'manual':
            ...
            InfoNotifier.InfoNotifier.g_progress_info.append("开始脸部抽取")
    ...
    InfoNotifier.InfoNotifier.g_progress_info.append("-------------------------")
    InfoNotifier.InfoNotifier.g_progress_info.append("处理图片 %d 张" % (images_found))
    InfoNotifier.InfoNotifier.g_progress_info.append("含脸图片 %d 张" % (faces_detected))
    InfoNotifier.InfoNotifier.g_progress_info.append("推荐手动删除不需要的脸部图片")
    InfoNotifier.InfoNotifier.g_progress_info.append("-------------------------")
