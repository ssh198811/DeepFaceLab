                ...
                if not debug:
                    ...
                    if not is_reached_goal:
                        ...
                        if iter_time >= 10:
                            ...
                            loss_string_chn = "{0}[#当前训练轮次{1:06d}][当前次训练时长{2:.5s}s]".format(time_str, iter, '{:0.4f}'.format(iter_time))
                        else:
                            loss_string = "{0}[#{1:06d}][{2:04d}ms]".format ( time_str, iter, int(iter_time*1000) )
                            loss_string_chn = "{0}[#当前训练轮次{1:06d}][当前次训练时长{2:04d}ms]".format(time_str, iter, int(iter_time * 1000))
                        ...
                        if shared_state['after_save']:
                            ...
                            for loss_value in mean_loss:
                                ...
                                if index_loss == 0:
                                    loss_string_chn +="源损失[%.4f]" % (loss_value)
                                    index_loss += 1
                                else:
                                    loss_string_chn += "目的损失[%.4f]" % (loss_value)
                        ...
                        InfoNotifier.InfoNotifier.g_loss_info = loss_string;
                        InfoNotifier.InfoNotifier.g_progress_info.append(loss_string_chn)
...
def main(**kwargs):
    ...
    if no_preview:
        ...
        while True:
            ...
            if key == ord('\n') or key == ord('\r') or UIParamReflect.UIParam2Config.bStopTrain:
                s2c.put ( {'op': 'close'} )
                UIParamReflect.UIParam2Config.bStopTrain = False
            elif key == ord('s') or UIParamReflect.UIParam2Config.bSaveOnce:
                s2c.put ( {'op': 'save'} )
                UIParamReflect.UIParam2Config.bSaveOnce = False
            elif key == ord('b'):
                s2c.put ( {'op': 'backup'} )
            elif key == ord('p'):
